...
export class ClientTraitor extends Space {
	...
	setup() {
		...
		const interactiveItem = new Entity();
		const interactiveSprite = Sprite.from(Texture.WHITE);
		interactiveItem.add(interactiveSprite);
		interactiveItem.add(Transform, { position: new Vector3(500, 600) });
		interactiveItem.add(Interactable);

		this.addEntities(ship, interactiveItem, player);
